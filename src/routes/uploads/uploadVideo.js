...
const maxFileSize = 16 * 1024 * 1024; // 10MB

// Set allowed file types
const allowedFileTypes = ["video/mp4", "video/3gpp", "video/x-msvideo"];
...
router.post("/", upload.single("image"), async (req, res) => {
  try {
    ...
    if (file.size > maxFileSize) {
      return Response400(res, "File size exceeds the maximum limit 20MB");
    }
    ...
    if (!allowedFileTypes.includes(file.mimetype)) {
      return Response400(
        res,
        "File type not allowed. Only mp4, 3gpp, x-msvideo allowed"
      );
    }
